...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Banner */}
      <div className="card p-6 bg-gradient-to-r from-brand-600 via-indigo-600 to-brand-800 text-white border-0 shadow-lg relative overflow-hidden">
        <div className="absolute right-0 top-0 -mt-4 -mr-4 h-32 w-32 rounded-full bg-white/10 blur-2xl" />
        <div className="flex flex-wrap items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2">
              <span className="badge bg-white/20 text-white text-xs font-semibold px-2.5 py-0.5">
            ...
            <p className="text-sm text-brand-100 mt-1">
              {isEmployeeOnly
                ? `Logged in with registered Payment Bank Email (${user?.email})`
                : "Manage organization staff, attendance, payroll, and branch operations."}
            </p>
          </div>

          <div className="flex gap-2">
            <button onClick={() => navigate("/attendance")} className="rounded-lg bg-white/20 px-4 py-2 text-sm font-medium backdrop-blur-xs hover:bg-white/30 transition">
              My Attendance
            </button>
            {isEmployeeOnly ? (
              <button onClick={() => navigate("/leaves")} className="rounded-lg bg-white text-brand-700 px-4 py-2 text-sm font-bold shadow-sm hover:bg-brand-50 transition">
                Apply Leave
              </button>
            ) : (
              <button onClick={() => navigate("/employees")} className="rounded-lg bg-white text-brand-700 px-4 py-2 text-sm font-bold shadow-sm hover:bg-brand-50 transition">
                + Add Employee
              </button>
            )}
          </div>
      ...
      {isEmployeeOnly && (
        <div className="card p-6 space-y-4 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-100 text-brand-700 dark:bg-brand-900/50 dark:text-brand-300 text-lg font-bold">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="badge bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 gap-1 text-[11px]">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 pt-1">
            <div className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40">
              ...
              <div className="min-w-0">
                <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Payment Bank Email</p>
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40">
              ...
              <div className="min-w-0">
                <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Assigned Branch</p>
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40">
              ...
              <div className="min-w-0">
                <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Date of Joining</p>
                <p className="text-sm font-medium text-slate-900 dark:text-white truncate">
      ...
      {stats && !isEmployeeOnly && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          ...
          <StatCard title="Attendance Rate" val={`${stats.attendance_percentage}%`} icon={CheckCircle2} color="text-teal-600" bg="bg-teal-50 dark:bg-teal-950/30" onClick={() => navigate("/attendance")} />
      ...
      {isEmployeeOnly && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="card p-5 space-y-2 cursor-pointer hover:border-brand-400 transition" onClick={() => navigate("/attendance")}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase text-slate-400">My Attendance Rate</span>
              <CheckCircle2 className="text-emerald-500" size={18} />
            </div>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">
              {stats ? `${stats.attendance_percentage}%` : "100%"}
            </p>
            <p className="text-xs text-slate-400">Click to view monthly Pagara attendance grid</p>
          </div>

          <div className="card p-5 space-y-2 cursor-pointer hover:border-brand-400 transition" onClick={() => navigate("/leaves")}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase text-slate-400">My Leave Balances</span>
            ...
            <div className="flex gap-4 pt-1">
              {balances.length > 0 ? (
                balances.slice(0, 3).map((b) => (
                  <div key={b.id}>
                    <p className="text-xs text-slate-400 capitalize">{b.leave_type.toLowerCase()}</p>
                    <p className="text-base font-bold text-slate-900 dark:text-white">{b.total - b.used} days</p>
                  </div>
                ))
              ) : (
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">12 Days Available</p>
              )}
            </div>
          </div>

          <div className="card p-5 space-y-2 cursor-pointer hover:border-brand-400 transition" onClick={() => navigate("/calendar")}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase text-slate-400">Company Calendar</span>
              <Calendar className="text-violet-500" size={18} />
            </div>
            <p className="text-sm font-medium text-slate-700 dark:text-slate-200 pt-1">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="space-y-2">
              {recent?.announcements?.map((a) => (
                <div key={a.id} className="p-3 rounded-lg border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-medium text-sm text-slate-900 dark:text-white">{a.title}</p>
                    <span className={`badge text-[10px] ${a.priority === "URGENT" ? "bg-red-100 text-red-700" : a.priority === "HIGH" ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"}`}>
                      {a.priority}
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {recent?.recent_moments?.map((m) => (
                <div key={m.id} className="p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-pink-50 text-pink-600 dark:bg-pink-950/30 text-lg">
                    ✨
                  </div>
              ...
              {(!recent?.recent_moments || recent.recent_moments.length === 0) && (
                <p className="text-sm text-slate-400 py-4 text-center col-span-2">No recent moments</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card p-5 space-y-3">
            <h3 className="font-bold text-sm text-slate-700 dark:text-slate-300">Quick Actions</h3>
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="space-y-3">
              {recent?.upcoming_events?.map((ev) => (
                <div key={ev.id} className="p-3 rounded-lg border border-slate-100 dark:border-slate-800 space-y-1">
                  <p className="font-medium text-xs text-slate-900 dark:text-white">{ev.name}</p>
...
function StatCard({ title, val, icon: Icon, color, bg, onClick, highlight }: {
  ...
  return (
    <div
      onClick={onClick}
      className={`card p-4 flex items-center gap-3 transition hover:shadow-md cursor-pointer ${
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 truncate">{title}</p>
        <p className="text-lg font-bold text-slate-900 dark:text-white leading-tight truncate">{val}</p>
      </div>
    </div>
  );
}
