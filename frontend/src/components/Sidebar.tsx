...
export default function Sidebar({ mobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  const sidebarContent = (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 overflow-y-auto">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
        ...
        {onCloseMobile && (
          <button onClick={onCloseMobile} className="md:hidden text-slate-400 hover:text-slate-600 p-1">
            <X size={18} />
          </button>
        )}
      </div>
            ...
            <div key={group.label || "main"} className="mb-1">
              {group.label && (
                <button
                  ...
                  className="flex w-full items-center justify-between px-3 py-1.5 text-[10px] font-semibold uppercase tracking-widest text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 transition"
                >
                  {group.label}
                  <ChevronDown
                    size={12}
  ...
  return (
    <>
      ...
      {/* Mobile Drawer */}
      {mobileOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden" onClick={onCloseMobile}>
